...
pub async fn create_question(app_data: Data<AppData>, body: web::Json<CreateQuestionArgs>) -> HttpResponse {
    let app_data= app_data.get_ref();
    let db = &app_data.db;

    match db.create_question(body.0).await {
        Ok(user) => {
...
pub async fn get_all_examiner_questions(app_data: Data<AppData>, body: web::Json<GetQuestionArgs>) -> HttpResponse {
    let app_data= app_data.get_ref();
    let db = &app_data.db;

    match db.get_all_examiner_questions(body.id).await {
        Ok(questions) => {
